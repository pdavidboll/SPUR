...
struct struct_spatial_i0_test scalar spatial_i0_test(real matrix Y, real matrix distmat, real matrix emat) {
    ...
    for (i = 1; i <= n_rho; i++) {
        ...
        if (rho > 0) {
            c = getcbar(rho, distmat)
            sigdm_ho = get_sigma_dm(distmat, c)
            om_ho = R' * sigdm_ho * R
        }
